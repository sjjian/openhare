...
import 'src/dialect/db2/lexer.dart';
import 'src/dialect/db2/parser.dart';
...
import 'src/dialect/db2/keyword.dart' as db2_keywords;

// 定义方言类型枚举
enum DialectType { mysql, oracle, pg, mssql, sqlite, redis, mongodb, duckdb, db2 }
...
Lexer createLexer(DialectType dialect, String content) {
  switch (dialect) {
    ...
    case DialectType.db2:
      return Db2Lexer(content);
  }
}
...
List<SQLChunk> splitSQL(DialectType dialect, String content,
  ...
  switch (dialect) {
    ...
    case DialectType.db2:
      return Db2Splitter(content).split(skipWhitespace: skipWhitespace, skipComment: skipComment);
  }
}
